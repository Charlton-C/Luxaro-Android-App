...
class SignUp : AppCompatActivity() {
                ...
                auth.createUserWithEmailAndPassword(email, password).addOnSuccessListener {
                    ...
                    if (it is FirebaseAuthUserCollisionException) {
                        binding.editTextTextEmailAddressLayout.error =
                            resources.getString(R.string.a_user_with_this_email_already_exists)
                    } else {
                        Toast.makeText(
                            this@SignUp,
                            resources.getString(R.string.sign_up_failed_LineBreak_please_try_again_later),
                            Toast.LENGTH_SHORT
                        ).show()
                        Log.e(
                            resources.getString(R.string.firebase_auth_error_colon_sign_up),
                            it.toString()
                        )
                    }
                    showSigningUpAnimation(false)
    ...
    private fun checkAllFields(): Boolean {
        ...
        if (binding.editTextTextNameInput.text.toString() == "") {
            binding.editTextTextNameLayout.error =
        ...
        if (binding.editTextTextPasswordInput.text.toString() == "") {
            binding.editTextTextPasswordLayout.error =
                resources.getString(R.string.please_enter_a_password)
        ...
        if (binding.editTextTextConfirmPasswordInput.text.toString() != binding.editTextTextPasswordInput.text.toString()) {
            binding.editTextTextConfirmPasswordLayout.error =
